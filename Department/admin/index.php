...
try {
    ...
    foreach ($countValue as $key => $tmp_data) {
        $queryResult = (isset($tmp_data[3]) && isset($tmp_data[4])) ? $dataBase->prepare("SELECT COUNT($tmp_data[0]) FROM $tmp_data[1] WHERE $tmp_data[3]==$tmp_data[4]") : $dataBase->prepare("SELECT COUNT($tmp_data[0]) FROM $tmp_data[1] ");
        $queryResult->execute();
...
<body class=" tw-flex-col tw-flex tw-p-20 tw-gap-y-10 ">
    ...
    <div class=" tw-mt-40 tw-w-28 tw-uppercase tw-rounded-md tw-p-2 sm:tw-text-xl sm:tw-block tw-hidden tw-bg-blue-700 tw-shadow-lg tw-shadow-blue-400 tw-text-white">Summary</div>
    ...
    <div class=" tw-mt-20 sm:tw-mt-40 tw-w-28 tw-uppercase tw-rounded-md tw-p-2 sm:tw-text-xl tw-text-lg tw-text-center tw-bg-blue-700 tw-shadow-lg tw-shadow-blue-400 tw-text-white">Details</div>
