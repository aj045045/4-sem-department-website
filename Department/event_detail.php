...
<br>
        <div class=" tw-bg-slate-300 tw-shadow-md tw-shadow-slate-500 tw-mx-20 tw-my-10 tw-p-10 tw-rounded-md tw-flex tw-flex-col tw-gap-y-10">
            <div class=" tw-text-center tw-text-4xl tw-pl-10 tw-capitalize md:tw-text-6xl tw-container" style="font-family: Brush Script MT;">event details</div>
            ...
            <div class=" tw-auto tw-text-center tw-capitalize md:tw-text-3xl tw-text-5xl tw-font-serif"><?php echo ucfirst($row['event_title']); ?></div>
            ...
            <div class="tw-flex tw-flex-row">
                <div class="tw-flex tw-flex-row tw-bg-slate-500 tw-shadow-md tw-shadow-slate-700 tw-text-white tw-rounded-md tw-p-2">
                    ...
                    <div class=" tw-font-serif tw-text-3xl"><?php echo ucfirst($row['event_venue']); ?></div>
                </div>
                <div class="tw-flex tw-flex-col tw-bg-slate-500 tw-shadow-md tw-ml-auto tw-shadow-slate-700 tw-text-white tw-rounded-md tw-p-2 tw-text-md">
                    <div class=" tw-italic tw-mx-3">From : <?php $dt = new DateTime($row['event_start']);
                                                            echo $dt->format('j-M-Y G:i A'); ?> <div>To : <?php $dt = new DateTime($row['event_end']);
                                                                                                            echo $dt->format('j-M-Y G:i A'); ?></div>
                    </div>
                </div>
            </div>

            <div class=" tw-text-lg"><?php echo $row['event_description']; ?></div>
            <div class=" tw-flex tw-flex-row tw-gap-x-6 tw-mx-auto">
                ...
                foreach ($rows as $row) {
                    echo '
                <img class="card-img-top tw-rounded-lg md:tw-block tw-hidden"  src="' . $row['photo_document'] . '" alt="Event' . $row['photo_document'] . '">
                ';
                }
                $resultData->closeCursor();
                ?>
